...
describe('loop/validate pipeline contract', () => {
  ...
  test('validation pins evidence to one commit and a blocked result never reaches a caller as a pass', async () => {
    ...
    expect(baseline).toMatch(/when `gh` fails \(a sandbox with no network\), take the default branch from `git symbolic-ref --short refs\/remotes\/origin\/HEAD`/)
    ...
    expect(baseline).toMatch(/never by itself leaves a claim ❓/)
