...
describe('complexity score band encoding', () => {
  ...
  test('the four-band routing matrix is stated consistently across docs and the pipeline', async () => {
    ...
    expect(validateIssue).toContain('| 0 | 0–24 | Opus 5 · medium | No | Sonnet 5 · xhigh (high at score ≤ 7) | Sonnet 5 · high |')
    ...
    expect(pipeline).toContain("{ name: '0–24', min: 0, max: 24, fableplan: false, validate: { model: 'opus', effort: 'medium' }, build: { model: 'sonnet', effort: 'xhigh' }, review: { model: 'sonnet', effort: 'high' } }")
