...
describe('complexity grading procedure', () => {
  ...
  test('a missing prefix, a differing prefix in either direction, or differing grades is an update, and a lower score lands only on evidence', () => {
    ...
    expect(decision).toMatch(/Yes for .*a rescore: a title with no `\[C<score>\]` prefix in a repository that follows that convention, a title prefix that differs from the recomputed score in either direction, or a rationale line whose grades differ from the traced ones/)
    ...
    expect(rules).toMatch(/A missing title prefix in a repository that follows the `\[C<score>\]` convention, a prefix that differs from the recomputed score in either direction, or a rationale line whose grades differ is an update/)
    ...
    expect(routing).toMatch(/every stage that reads the issue after the edit lands routes on it/)
    expect(routing).toMatch(/A read-only validator rescore that writes no issue edit never lowers routing/)
  })
  ...
  test('the Execution block restamps upward on a higher score and lowers only unchanged band defaults on a lower score', async () => {
    ...
    expect(decision).toMatch(/`fableplan:` field is a routing signal: `yes` when the title score or the recomputed score is 71 or higher\. It keeps the title floor for this run's plan decision/)
    expect(decision).toMatch(/a loop that applies it before the build routes its later stages, such as the first review, on the lower score/)
