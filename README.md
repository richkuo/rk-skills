...
| `milestoneplan` | Read-only pre-flight for a milestone: audits every `## Execution` block against the complexity band formula, closes the dependency graph, derives execution waves and the critical path, projects run size, and returns a go / no-go verdict. Never edits an issue — hands fixes to `execution-plan-review` and the run to `milestone-workflow`. |
