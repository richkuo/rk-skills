...
Every issue's first line also carries an explicit **`fableplan: yes|no`** signal, so later steps read the planning decision instead of re-deriving it from the score. It's `yes` at Capability ≥ 2 / score ≥ 50 — a Fable 5 plan is posted before the build; the builder is Opus 5 at both bands: high at band 2, xhigh at band 3. Effort runs `high`/`xhigh` for Opus and Sonnet builds; Fable builds may also run at `medium` or, at the planner's discretion, `low` for issues lighter than the formula's own floor. Fable 5 never runs at `xhigh` — `high` is its ceiling on every stage (build, plan, validate, review, fix).
