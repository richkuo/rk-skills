...
| `milestoneplan` | Read-only pre-flight for a milestone: classifies every issue into build / resume / skip, renders each issue's Execution-block routing (score, dependency fields, build model, efforts, fableplan, plan effort, first review) and flags stamps that contradict the complexity band formula, closes the dependency graph (cycles, cross-milestone and closed-predecessor edges, resolved by real merge state), derives the execution waves and critical path, projects the run's agent count and per-model mix, and returns a go / no-go verdict. Always renders the findings and per-issue audit as markdown tables, never prose lists. Never edits an issue — hands fixes to `execution-plan-review` or `validate-issue` and the run to `milestone-workflow`. |
