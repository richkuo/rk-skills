...
| `execution-plan-review` | Renders the ordering/model/effort/fableplan table from the issues themselves, takes revisions ("11 should be medium", "12 runs after 11"), rejects cycles across both edge kinds, pushes back when a revision leaves a non-Fable build at `low`/`medium` effort, and writes changes back to the issues. |
