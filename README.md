...
**The reviewer follows the complexity band.** `fix-pr-review`, the `-loop` skills, `milestone-workflow`, and the Action prompts all pick the reviewer from the issue's or PR's `[C<score>]`: `@claude review` at C0–C30, `@claude opus review` at C31–C70, and `@claude fable review effort:high` at C71+ or when no score is available. The reviewer escalates on its own, coarser scale, so these boundaries deliberately differ from the validate/build bands in `validate-issue` step 6; that skill owns both tables. A stamped `PR review:` trigger in an issue's Execution block overrides the band. Fable reviews the first cycle only, and the blocking re-reviews after it step down one rung each: `@claude opus review` for the first, `@claude review` for every one after that. A first review at C31–C70 keeps opus for every blocking re-review, and a pass that addressed only non-blocking items always drops to `@claude sonnet review` without consuming a rung. With Codex selected, both heavy tiers collapse onto a bare `@codex review`.
