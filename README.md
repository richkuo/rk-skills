...
| `milestoneplan` | Read-only pre-flight for a milestone: renders each issue's Execution-block routing (score, dependency fields, build model, efforts, fableplan, plan effort, first review) and recommends field changes wherever a stamp contradicts the complexity band formula. Never edits an issue — hands fixes to `execution-plan-review` or `validate-issue` and the run to `milestone-workflow`. |
