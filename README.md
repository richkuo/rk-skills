...
**The reviewer follows the complexity band.** `fix-pr-review`, the `-loop` skills, `milestone-workflow`, and the Action prompts all pick the reviewer from the issue's or PR's `[C<score>]`: `@claude sonnet review` at C0–C10, `@claude review` at C11–C40, `@claude opus review` at C41–C80, and `@claude fable review effort:high` at C81+ or when no score is available. The reviewer escalates on its own, coarser scale, so these boundaries deliberately differ from the validate/build bands in `validate-issue` step 6; that skill owns both tables. A stamped `PR review:` trigger in an issue's Execution block overrides the band. Fable reviews the first cycle only, and the blocking re-reviews after it step down one rung each: `@claude opus review` for the first, `@claude review` for every one after that. The ladder stops at `@claude review` and never drops to sonnet. That step-down keys to the reviewer that actually ran cycle 1, and the score band does not decide it, so a stamped Fable first review steps down at any score, while a first review on any other model keeps its own trigger for every blocking re-review, and a pass that addressed only non-blocking items always drops to `@claude sonnet review` without consuming a rung. With Codex selected, the C11+ bands collapse onto a bare `@codex review`, while the C0–C10 band and the non-blocking re-review keep `@codex luna review`.
