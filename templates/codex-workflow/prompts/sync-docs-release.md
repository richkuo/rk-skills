...
3. Classify each in-scope commit and update the affected docs surgically. AGENTS.md is the dense agent-facing reference describing current behavior, and CLAUDE.md is kept byte-identical to it when the repository ships both. README.md is the public entry point and changes only when the public surface changes. Edit only the sections that actually changed; do not rewrite whole files.
