...
- Obey the conventions and invariants documented in AGENTS.md, and in CLAUDE.md as well when the repository keeps both files in sync. Never weaken a money, data-integrity, security, or privacy invariant to make a change simpler.
...
3. Classify each in-scope commit and update the affected docs surgically. AGENTS.md is the dense agent-facing reference describing current behavior. Before you edit it, run readlink on AGENTS.md to learn what it is. When readlink reports CLAUDE.md, the two paths are one file: apply each documentation edit exactly once through CLAUDE.md and make no second edit through AGENTS.md, because a repeated insert lands twice in the canonical file. When AGENTS.md is a real separate file and the repository ships CLAUDE.md as well, keep CLAUDE.md byte-identical to AGENTS.md and apply each edit to both. README.md is the public entry point and changes only when the public surface changes. Edit only the sections that actually changed; do not rewrite whole files.
...
6. After editing, check the size of AGENTS.md by running wc with the -c flag on it. If it exceeds 40000 bytes, condense it in place back under 38000 bytes without splitting it into multiple files.
