...
This format overrides any final-comment format the code-review skill specifies. Keep that skill's multi-agent review process; replace only the shape of the posted comment.

Never execute the project's code on this route, in any mode: no test suites, builds, type checks, simulations, or scripts. Judge correctness by reading the code.
...
- Run a counterfactual closure pass after drafting: assume each fix is applied as written, re-read the full diff, and add each defect that survives. Stop only when a full pass adds nothing. Report every surviving finding in this single comment.

## Comment format

The comment contains nothing outside this structure: no preamble, summary, header, emoji, or footer outside it. Write it as direct instructions for an agent that will act on it.

### Verdict

- First line: exactly LGTM or Needs Updates.
- Verdict keys off blocking sections only: ### Needs Fixing and ### Requires Human Review block; ### Recommended Optional and ### Create Follow-up Issue do not. Needs Updates iff at least one blocking item, else LGTM.
...
Apply these rules in order. They never remove a finding's eligibility for ### Requires Human Review.
...
- Every finding goes under exactly one H3 section; omit empty sections. Numbered items: a bold one-sentence title, newline, description with file:line and why.
