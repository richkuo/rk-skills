...
- **PR title convention:** `type(scope): summary [C<score>, <model>, <effort>]` — Conventional Commits `type` (`feat`/`fix`/`refactor`/`chore`/`docs`/`ci`/`test`/`perf`/`style`) and `scope` (`#<issue>` when the PR closes one, else a short component name, or omit the scope entirely when neither fits), followed by the lowercase imperative summary, then a trailing `[C<score>, <model>, <effort>]` bracket (append `, fableplan` only when a **Fable 5** plan drove the build — posted before the build or adopted from the issue thread per `work-on-issue` step 0.1; a maintainer-written plan earns no marker). E.g. `fix(#95): resolve double-fill race on order matching [C95, Opus 5, xhigh, fableplan]`. When the PR closes an issue, reuse the score from the issue's `[C<score>]` title prefix and pair it with the model/effort actually used to build the PR; derive both fresh via the `validate-issue` step 6 formula for standalone PRs.
...
- **Worktree/branch names carry a coding-agent prefix**: `cc/` for Claude Code, `cursor/` for Cursor, `codex/` for Codex — e.g. `cc/issue-873-scale-in-pyramiding`. On Claude Code, use the native `EnterWorktree` tool and pass the `cc/`-prefixed name directly. **The tool normalises that name rather than using it verbatim** — it has been observed to replace `/` with `+` and to prefix the branch with `worktree-` — so take the real path from the tool's result and the real branch from `git branch --show-current`; never rebuild either from the name you passed. On Cursor/Codex (no `EnterWorktree` tool), use `git worktree add .worktrees/<prefix>/<slug> -b <prefix>/<slug> <base>` and add the `cursor/`/`codex/` prefix by hand. Worktrees you create yourself go under the repo's harness-neutral `.worktrees/`, never inside another agent's `.claude/`; keep `.worktrees/` gitignored.
