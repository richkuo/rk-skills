...
5. On a non-zero exit, first check for work the failed run already landed (${isFix ? 'a head commit newer than the pre-run head on the PR, or a new disposition comment' : 'a \`' + harness.branchPrefix + 'issue-<issue>-*\` branch on the remote, or an open PR closing the issue'}): landed work is a completed pass, so continue with step 6 and verify it under step 8 instead of re-running. Only when nothing landed, retry the shim once with the same inputs; a second failure is a blocker. A retry never produces a second PR, issue-body edit, or disposition comment.
6. After every run, pass or fail, read the CLI's final message and the event log. When the output names the model that served the run, compare it with \`${ex.cli_model}\` — a different model is a substitution: report it ${isFix ? 'in the summary' : 'in flags and in the summary'}, never as a ${modelName} ${isFix ? 'fix pass' : 'build'}. An output that names no model is recorded as model unverified beside the requested id. This step is never skipped on a zero exit.
7. Diff \`git status --porcelain --untracked-files=all\` in the main checkout against the snapshot, ignoring every path under \`.claude/worktrees/\` (a concurrent issue's worktree lives there and is not a stray write); report any other change outside the issue's worktree ${isFix ? 'in the summary' : 'in flags'}.
8. ${verifyStep}
9. ${reviewStep}
...
const BANDS = [
  ...
  { name: '50–70', min: 50, max: 70, fableplan: false, validate: { model: 'opus', effort: 'xhigh' }, build: { model: 'opus', effort: 'xhigh' } },
  { name: '71–80', min: 71, max: 80, fableplan: true, validate: { model: 'fable', effort: 'medium' }, build: { model: 'opus', effort: 'xhigh' } },
...
const CLAUDE_STEP_DOWN_TRIGGER = [
  [/^@claude\s+fable\b/, '@claude opus review effort:high'],
...
- validate_model: from an optional "**Validate model:**" line — map "Fable 5.1"→fable and "Opus 5" (any Opus)→opus. When the line carries a parenthetical naming an external harness — "Astra (Codex CLI, gpt-6-astra)", "Luna (Codex CLI)" — map "(Codex CLI…)"→codex and "(Cursor CLI…)"→cursor, set validate_model_name to the name before the parenthetical, and set validate_cli_model to the id after the comma inside the parenthetical when one is present; OMIT validate_cli_model when the parenthetical carries no id, and OMIT both fields for Claude models. When the line is absent, OMIT validate_model — absence means the runtime derives the validate model from the [C..] band. Never read a model from the "Validate effort:" line
