...
3. If GitHub reports the branch behind the base and the repo requires up-to-date branches: run \`gh pr update-branch ${prNumber}\` ONLY when it merges cleanly, then repeat the CI gate on the new head. If it reports conflicts, do NOT resolve them — return merged false, blocker "merge conflict with the base branch".
4. Merge: \`gh pr merge ${prNumber} --squash --delete-branch\`, adding \`--match-head-commit ${head.sha}\` when the head is unchanged since step 1.
