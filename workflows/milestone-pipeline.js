...
4. Otherwise invoke the \`fix-pr-review\` skill with the PR number and follow it exactly: re-validate each finding, fix or refute it, push, post dispositions, re-trigger through the skill's step-7 routing (never repeat the \`@claude fable review\` trigger — fable reviews the first cycle only, so a blocking re-trigger is the standard \`@claude review\`), and wait for that re-review verdict.
...
// A fable first review is first-review-only: every blocking re-review after it
// runs the standard bare-@claude reviewer (model null) — fable never
// re-reviews.
...
async function runSubagentReviewLoop(issue, prNumber, ex, validation, plan) {
  ...
  // Fable reviews the first cycle only. Every blocking re-review after a fable
  // first review runs the standard bare-@claude reviewer instead of repeating
  // fable — independent of the cycle index.
  const blockingReview = firstReview.model === 'fable' ? { model: null, effort: 'high' } : firstReview
  const notes = []
  ...
  while (cycles < MAX_REVIEW_CYCLES) {
    ...
    nextReview = review.blocking_count === 0 ? { model: 'sonnet', effort: 'high' } : blockingReview
  }
