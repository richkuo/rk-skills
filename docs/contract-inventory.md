...
| Band-derived review trigger | `validate-issue` step 6 band table (review column); `fix-pr-review-loop` step 1 restates it for the PR entry points | `fix-pr-review-loop` (first review), `work-on-issue-loop` (first review), `fix-pr-review` (blocking re-review), `workflows/milestone-pipeline.js` (`BANDS` + `firstReviewTrigger`), `prd-to-issues`, `milestoneplan`, `milestone-workflow`, `templates/claude-workflow/prompts/issue-workflow.md` (first review) and `templates/claude-workflow/prompts/fix-pr.md` (re-review) | The reviewer follows the complexity band: `@claude review` at C0–C20, `@claude opus review` at C21–C80, `@claude fable review effort:high` at C81+ or with no score; a stamped `PR review:` trigger overrides the band; Fable reviews the first cycle only, so the blocking re-reviews after it step down one rung each — `@claude opus review` for the first, `@claude review` for every one after that — while a C21–C80 first review keeps opus for every blocking re-review; the cheap non-blocking re-review (`@claude sonnet review`) ignores the band and consumes no rung | Where the score is read (issue title prefix, PR title bracket, Execution block) per entry point; with Codex selected both heavy tiers collapse onto a bare `@codex review` | `tests/pr-review-contract.test.js` (skill + Action-prompt consumers); `tests/milestone-pipeline.test.js` (`firstReviewTrigger`) |
