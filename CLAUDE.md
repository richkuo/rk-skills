...
- **PR title convention:** `type(scope): summary [C<score>, <model>, <effort>]` — Conventional Commits `type` (`feat`/`fix`/`refactor`/`chore`/`docs`/`ci`/`test`/`perf`/`style`) and `scope` (`#<issue>` when the PR closes one, else a short component name, or omit the scope entirely when neither fits), followed by the lowercase imperative summary, then a trailing `[C<score>, <model>, <effort>]` bracket (append `, fableplan` only when a **Fable 5** plan drove the build — posted before the build or adopted from the issue thread per `work-on-issue` step 0; a maintainer-written plan earns no marker). E.g. `fix(#95): resolve double-fill race on order matching [C95, Opus 5, xhigh, fableplan]`. When the PR closes an issue, reuse the score from the issue's `[C<score>]` title prefix and pair it with the model/effort actually used to build the PR; derive both fresh via the `validate-issue` step 6 formula for standalone PRs.
...
- **Before creating or editing any GitHub issue, load the `github-issue-format` skill** — it defines the mandatory `[C<score>]` title convention, complexity rationale line, complete-body rule, and the mandatory `## Plain simple English` section. Never file an issue without it.
- **Issue body order:** complexity rationale line, `## Problem`, `## Goal`, `## Approach`, `## Acceptance criteria`, `## Plain simple English`, then any Execution block, then the attribution footer. The plain-language section is mandatory on every issue — one short paragraph under 55 words in ASD-STE100, no jargon, no unexplained acronyms, no file paths — stating what is wrong or missing and why it matters, so a human understands the issue without reading the technical sections. An edit that rewrites a body adds the section when it is missing. (`github-issue-format` owns the rule; `new-issue` and the issue-editing procedure enforce it.)
