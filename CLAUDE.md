...
- **Worktree/branch names carry a coding-agent prefix**: `cc/` for Claude Code, `cursor/` for Cursor, `codex/` for Codex — e.g. `cc/issue-873-scale-in-pyramiding`. `EnterWorktree` adds the `cc/` prefix itself — pass it an unprefixed name (`issue-873-…`). Tools/workflows without that auto-prefixing (raw `git worktree add`) must add the prefix by hand.
