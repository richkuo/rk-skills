...
- **PR body order:** `## Summary` / verification first — keep those scannable, don't restate the whole issue. End with `## Plain simple English` — the Response Style block — stating what changed and why it matters. (`work-on-issue` enforces this when opening PRs.)
