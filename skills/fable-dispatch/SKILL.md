...
- **The calling skill supplies the `--allowedTools` list**: every read-only command its procedure needs, named explicitly — e.g. `"Bash(gh issue view *)" "Bash(gh pr list *)" "Bash(git log *)"`, with `-R owner/repo` forms when the procedure reads another repo. Plan mode auto-approves built-in reads, and a permission classifier can approve other read-only commands (observed live once), but classifier approval is heuristic and a headless run has no user to answer a prompt — the explicit list is what makes the procedure's reads deterministic. The list carries read-only commands only: never a bare `Bash`, never an edit, comment, or push command; plan mode keeps blocking writes either way.
