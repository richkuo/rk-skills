...
- For fan-out runs, merge each worker branch collected in step 3 (`cc/fable-orchestrate/<short-task-name>/worker-<n>`) into the task branch yourself — `git -C <task-worktree> merge <worker-branch>` — and resolve conflicts (you pinned the interfaces; you own the seams). A worker that returned "no changes" has nothing to merge; skip it without error. Two workers touching the same file despite disjoint specs is a seam defect: resolve the conflict yourself, never re-dispatch it.
