...
- PR review: the pipeline derives the first-review trigger from the score on the first-review scale, whose boundaries differ from the build bands above — the standard `@claude review` at 0–30 (no pinned model), `@claude opus review` at 31–70, `@claude fable review effort:high` at 71+ or with no score. The fable trigger is first-review-only, and the blocking re-reviews after it step down one rung each — `@claude opus review` for the first, `@claude review` for every one after that. Stamp an explicit `@claude <model> review effort:<tier>` line only to override that default.
