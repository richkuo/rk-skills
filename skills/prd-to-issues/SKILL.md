...
- PR review: the pipeline derives the first-review trigger from the band — the standard `@claude review` at 0–20 (no pinned model), `@claude opus review` at 21–80, `@claude fable review effort:high` at 81+. The fable trigger is first-review-only: the blocking re-review after it is `@claude opus review`, and blocking re-reviews after that opus pass are the standard `@claude review`. Stamp an explicit `@claude <model> review effort:<tier>` line only to override that default.
