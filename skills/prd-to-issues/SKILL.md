...
- PR review: the line stays the standard `@claude` review trigger. The pipeline derives the first-review default from the band, cross-model where a stronger tier exists (Opus 5 · high at 0–49, Fable 5 · high at 50+); stamp an explicit `@claude <model> review effort:<tier>` trigger only to override that default.
