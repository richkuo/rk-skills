...
- PR review: the line stays the standard `@claude` review trigger. The pipeline derives the first-review default from the band (Opus 5 · high at 0–74, Fable 5 · high at 75+); stamp an explicit `@claude <model> review effort:<tier>` trigger only to override that default.
