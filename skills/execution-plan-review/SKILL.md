...
| Revision names a validate model or a validate effort | Neither is stampable — validation is fully derived from the `[C<score>]` band (Opus 5 · medium at 0–24, Opus 5 · high at 25–49, Fable 5 · high at 50+ or no prefix). Say so and drop the revision; if they want the heavier pass, the score is what to revisit. Offer to strip a legacy `Validate effort:` line when one is present |
| A stamped Build model or Effort diverges from the issue's current `[C..]` band default | Flag it in the table with the band value (a re-scored prefix over an old stamp is the usual cause) and ask once whether to re-derive from the band or keep the stamp as a deliberate override — never silently rewrite it |
| A revision flips fableplan `Yes` → `No` on a band-2 (`[C50]`–`[C74]`) issue | Recommend restoring the build effort to `xhigh` in the same batch — band 2 builds at high only because a posted Fable plan precedes the build; without the plan the harder issue must not run at less effort than band 1 |
