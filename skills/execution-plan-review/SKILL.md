...
| A revision flips fableplan `Yes` → `No` on a band-5 (`[C71]`–`[C80]`) issue | Recommend restoring the build effort to `xhigh` in the same batch — band 5 builds at high only because a posted Fable plan precedes the build; without the plan the harder issue must not run at less effort than band 3 |
