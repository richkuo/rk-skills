...
Cell rules:
- **Validate** is the effective `<model> · <effort>` the pipeline dispatches. The model derives from the `[C<score>]` band (a missing prefix routes as band 5) unless a `Validate model:` line stamps `Fable 5.1`, `Opus 5`, or `<Name> (Codex CLI[, <model-id>])`; the effort is the band default unless a `Validate effort:` line stamps one. The `validate-issue` step 6 band table is the authority; do not restate it here. Show a stamp as `Fable 5.1 · low (stamped; band Opus 5 · high)` or `Astra · low (Codex CLI, gpt-6-astra; stamped; band Fable 5.1 · high)`; show a clamp as `Opus 5 · high (stamped <tier> → high: low/medium are Fable-only)`.
...
| Revision names a validate model | Write `- **Validate model:** Fable 5.1`, `Opus 5`, or `<Name> (Codex CLI[, <model-id>])`; a stamped model overrides the band (`validate-issue` step 6). A revision that names the band default removes the line. A Cursor CLI validate is rejected: the pipeline runs a validate pass only in the Codex read-only sandbox, so say so and keep the prior stamp. Any other Claude name is asked about in half a sentence |
