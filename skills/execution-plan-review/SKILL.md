...
- The user revises in shorthand ("11 should be medium", "12 depends on 8 and 9", "13 runs after 12", "clear 14's dependencies", "plan 17 at medium", "validate 271 at medium"). Resolve ambiguous references (row position vs issue number) against the table just shown; when genuinely ambiguous, confirm in half a sentence. A bare effort revision ("11 should be medium") means the **build** effort — confirm in half a sentence when the issue also has a plan stage. A revision that names an external CLI ("build 275 with luna on codex at max", "277 grok on cursor high", "278 on cursor with cursor-grok-4.6-high") writes `- **Build model:** <Name> (Codex CLI)` or `<Name> (Cursor CLI)`, appending `, <model-id>` inside the parenthetical when the user names an id, and writes the tier to `Effort:`; `max` is a Codex CLI-only tier (a Cursor `max` is written as `xhigh`, and a Claude `max` as `xhigh` on Opus/Sonnet or `high` on Fable — say the clamp applied). Only `Luna` on Codex and `Grok` on Cursor have a default model id; any other name needs an explicit id, so ask for one in half a sentence before writing. "validate <n> at <tier>" writes a `- **Validate effort:** <tier>` line; "plan <n> at <tier>" writes a `- **Plan effort:** <tier>` line. A revision that names the band default for that stage removes the line instead of writing a redundant stamp, and say so. A milestone-wide revision ("every Fable validate at medium", "all plans at medium") applies to every issue whose stage runs on Fable, in one batch.
...
| Revision would put a non-Fable build's effort at `low` or `medium` | Set `high`, or switch the build to Fable 5.1 if that tier was the point — Opus/Sonnet run at high/xhigh only. A Codex CLI or Cursor CLI build accepts `low`, `medium`, `high`, and `xhigh` as stamped, and Codex also accepts `max` |
| Revision names an external CLI build without a known default model id | Only `Luna (Codex CLI)` and `Grok (Cursor CLI)` resolve an id on their own; ask for the id in half a sentence and write it inside the parenthetical, because the pipeline blocks an unknown name rather than guessing |
...
| Revision would put any Fable 5.1 stage at `xhigh` | Write it as stamped. The user's revision is the explicit ask that permits `xhigh` on Fable; the default without a stamp stays `high` (the LLM Attribution Footer section of CLAUDE.md owns this rule) |
...
| Revision names a validate effort ("validate 271 at medium") | Write the `Validate effort:` line. On a Fable validate (`[C71]`+ or no prefix) every tier from `low` to `xhigh` runs as stamped. On an Opus validate (`[C0]`–`[C70]`) `high` and `xhigh` run as stamped, and `low` or `medium` is raised to `high` — say so, because those tiers are Fable-only, and offer the score as the way to reach a Fable validate |
| A stamped Build model or Effort diverges from the issue's current `[C..]` band default | Restamp to the band default and say so in the report (a re-scored prefix over an old stamp is the usual cause; the pipeline re-routes a build only when its own validator outranks the title band, so once the title carries the higher score it builds on the stale stamp — restamping now is what corrects the routing). Keep the stamp only when the user names it a deliberate override in this session. A stamp on the Codex CLI or the Cursor CLI that the user named in this session is such an override; report it in the table and leave it, and a run-time rescore keeps the harness and only adds fableplan |
| A revision flips fableplan `Yes` → `No` on a band-4 (`[C71]`–`[C80]`) issue | Write it and say the issue loses its Fable plan. The build effort stays `xhigh`, the same as band 3, so no effort change follows |
...
| Revision names a plan effort ("plan 271 at medium") | Write the `Plan effort:` line with `low`, `medium`, `high`, or `xhigh`. A revision to `high` removes the line, since high is the default |
