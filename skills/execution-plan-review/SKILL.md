...
Cell rules:
- **Validate** is the effective `<model> · <effort>` the pipeline dispatches. The model derives from the `[C<score>]` band (a missing prefix routes as band 5) unless a `Validate model:` line stamps `Fable 5.1` or `Opus 5`; the effort is the band default unless a `Validate effort:` line stamps one. The `validate-issue` step 6 band table is the authority; do not restate it here. Show a stamp as `Fable 5.1 · low (stamped; band Opus 5 · high)`; show a clamp as `Opus 5 · high (stamped <tier> → high: low/medium are Fable-only)`.
...
| Revision names a validate model | Write `- **Validate model:** Fable 5.1` or `Opus 5`; a stamped model overrides the band (`validate-issue` step 6). A revision that names the band default removes the line. Any other name is asked about in half a sentence, because the pipeline reads only those two |
| Revision names a validate effort | Write the `Validate effort:` line. A Fable validate (stamped, `[C71]`+, or no prefix) runs every tier `low` to `xhigh` as stamped. An Opus validate (stamped, or `[C0]`–`[C70]` unstamped) runs `high` and `xhigh` as stamped and raises `low` or `medium` to `high`; say so, and offer a `Validate model:` stamp as the way to reach a Fable validate |
