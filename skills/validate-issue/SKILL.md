...
The first review is the `@claude` GitHub Action posted on the PR: the standard `@claude review` trigger at bands 0–1 (the Action's configured default model), `@claude opus review` at bands 2–4, `@claude fable review effort:high` at band 5 — first review only: the blocking re-review after a fable review is `@claude opus review`, and blocking re-reviews after that opus pass are the standard `@claude review`. In subagent review mode the same models apply, with bands 0–1 inheriting the session default. Sonnet never takes a first review; it appears only as the cheaper re-review after a fix pass that addressed nothing blocking.
