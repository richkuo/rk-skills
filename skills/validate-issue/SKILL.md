...
**Each proposed split/child is itself fully specified — never recommend filing a stub.** Per the repo's issue rules, every spun-off issue needs its own complexity-prefixed title, problem statement, and acceptance criteria before it's filed. If a part isn't ready to spec, recommend tracking it as a checklist line in the parent, not a separate issue yet. Don't actually file the children during validation — propose them; filing happens only if the user says to act on the disposition.
