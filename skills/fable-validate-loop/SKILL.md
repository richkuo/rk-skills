...
**Capability-3 note:** this gate runs fableplan for band-3 issues (score ≥ 75), and their verdict line reads `fableplan: yes` — the signal is `yes` for all of Capability ≥ 2. This loop implements via work-on-issue-loop on the session model, so the posted Fable plan is the only guaranteed Fable 5 involvement for a band-3 issue here — skipping it would drop Fable from the hardest issues entirely.
