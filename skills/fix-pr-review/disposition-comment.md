...
- **The header lists every source this pass addressed**, each review or comment as `<author> · <timestamp>`. [fetch-recipes.md](fetch-recipes.md) matches older feedback against these lists, so a source missing from every header is collected again.
