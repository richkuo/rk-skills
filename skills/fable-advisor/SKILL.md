...
Record the issue number and URL — steps 3, 8, and 9 need them. If no issue is referenced, skip this and every later issue-posting step.
...
Save the plan verbatim to a scratchpad file immediately, so it survives context summarization during a long build and step 3 can post it exactly as produced.
...
### 3. Post the plan to the GitHub issue (only if one was resolved in step 1)
...
### 4. Set up an isolated git worktree

Same as any build task in this environment: never build in the user's checkout. Create a fresh worktree/branch off `origin/<default>` named `cc/fable-advisor/<short-task-name>` (via `EnterWorktree`, name passed verbatim; fetch first and verify the worktree HEAD matches `origin/<default>` with `git -C <worktree-path> rev-parse HEAD origin/<default-branch>`; if the SHAs differ on the worktree you just created, move it onto the fetched default with `git -C <worktree-path> reset --hard origin/<default-branch>` — safe only because the brand-new branch carries no commits, and always anchored with `-C <worktree-path>` so it can never touch the original checkout). If the directory isn't a git repo, ask the user how to proceed. All of steps 5–7 happen inside the worktree.

### 5. Build, consulting the advisor at fixed checkpoints

The executor (you) builds per the plan, working uninterrupted between checkpoints. Consult the advisor — SendMessage to the step-2 agent, never a new Agent call — only when a checkpoint fires:
...
Run the repo's tests/verification relevant to the change and confirm the task's acceptance criteria are met. Do not request the pre-commit review on a build you haven't verified yourself — the reviewer judges correctness, it is not your test runner.

### 7. Binding pre-commit review (fresh reviewer, never the advisor)

Spawn a **new one-shot** Fable 5 reviewer — fresh context so it isn't anchored on the plan it would otherwise have authored:
...
On approval, commit and push from the worktree branch and open a PR per the repo's conventions. When an issue was resolved in step 1, include `Closes #<N>` in the PR body (fully-qualified `Closes owner/repo#N` for cross-repo issues). The PR body includes a short **Advisor log**: the plan source, each checkpoint consult (question → recommendation → disposition), any overruled advisory findings, and the review verdict trail. Footer per convention, naming the executor model actually in use:
