...
- **Every finding** (all four sections) must then include **Plain simple English:** — plain simple English under 55 words, no jargon, no unexplained acronyms — so a human can understand the finding without reading the technical paragraph. For `### Requires Human Review`, prefer a concrete A/B decision question; for other sections, state what is wrong or what should change in everyday language.
