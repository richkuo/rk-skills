...
Show numbered tracks with titles, hard `after` edges apart from `runsAfter` edges, inferred edges, each issue's model/effort/fableplan (fableplan issues plan at the stamped Plan effort, else high), the readiness boundary, and merge order. **Do not invoke the Workflow tool until the user approves this plan**; the approval is the multi-agent opt-in.
...
| A remaining PR conflicts after a merge | Resolve it in-session under step 5 sub-step 2; `.md`-only fixes keep the LGTM, any other file needs a sonnet re-review |
