...
- **Files that instruct an agent are executable** (`.claude/**`, `.agents/**`, prompts, skills, CI config, schemas): review them for behavioral defects and self-consistency.
...
- **Bind the verdict to one revision.** On a staged-snapshot route, judge that snapshot only and never fetch or review a newer head, even when your tools permit it; the caller binds the verdict to the staged head. On any other route, recheck the live head and base before you deliver. If either moved, review the new changes before you emit `LGTM`; if you cannot, emit `Needs Updates` with a `### Requires Human Review` item that names the reviewed and current commits. This applies with no findings too. When the head moved and a finding exists, name its short SHA once in the first finding.
...
In an interactive session, the Response Style definition is the trusted CLAUDE.md/AGENTS.md one, and the comment ends with the **LLM Attribution Footer**, verb **Reviewed**.
