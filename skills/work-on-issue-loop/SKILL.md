...
**Which trigger to post:** derive it from the issue's complexity score with the first-review table in `validate-issue` step 6, which owns every boundary and both the Claude and the Codex column; fix-pr-review-loop step 1 points at that same table. This file states no boundary of its own — load the owner table and read the row for this score. The issue you just implemented is the score source: its `[C<score>]` title prefix, or a stamped `PR review:` line in its Execution block, which overrides the band. A missing score routes to the heaviest row.

**Map a stamped model before you post it — never pass it through verbatim.** `claude.yml` resolves only `opus`, `sonnet` and `fable` (each with a `5` suffix), so a stamped `sonnet` or `haiku` posts `@claude sonnet review`, a stamped `opus` posts `@claude opus review effort:high`, and a stamped `fable` posts `@claude fable review effort:high`, each keeping the stamped `effort:<tier>` in place of `high` when one is given. A shorthand the Action does not admit becomes its route keyword, and on a trusted-author PR that selects the write-capable fix-pr job — which pushes commits — instead of the review this loop waits for. With Codex selected, map onto the Codex column instead (`sonnet`/`haiku` → `@codex luna review`, `opus`/`fable` → the bare `@codex review`); fix-pr-review-loop step 1 owns both mappings.
...
- **fix-pr-review-loop step 3** — check the review against the merge-conflict check and the stop conditions: a bare LGTM with nothing left to fix stops the loop; when `review_count > 5`, stop at the first LGTM even if non-blocking findings remain; a `Needs Updates` verdict never stops the loop by cycle count alone. Its one exception is the divergence brake, which needs a second condition as well as the count: `pr_cycle_count >= 4` **and** `Needs Updates` **and** every blocking finding sitting in code an earlier cycle added. `pr_cycle_count` is fix-pr-review step 4's growth-check count derived from the PR's trigger comments, never the in-memory `review_count` that the past-5 LGTM cap reads. A blocking finding that cannot be attributed to a cycle — no `file:line`, or a path the head no longer has — defeats the second condition instead of satisfying it, and a line a base merge brought in counts as base-branch work, not as a cycle's own. That combination stops the loop and reports per step 5's **Diverging** row, because a loop correcting its own corrections compounds the PR rather than converging it. A `Needs Updates` whose findings are still in the PR's original work keeps going however many cycles it takes.
