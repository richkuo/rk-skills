...
**Which trigger to post:** derive it from the issue's complexity score with fix-pr-review-loop step 1's band table — `@claude review` at C0–C20, `@claude opus review` at C21–C80, `@claude fable review effort:high` at C81+ or when no score is discoverable. The issue you just implemented is the score source: its `[C<score>]` title prefix, or a stamped `PR review:` line in its Execution block, which overrides the band.
