...
- **fableplan signal:** `· fableplan: yes` **when Capability ≥ 2** (score ≥ 50 — a Fable 5 plan is posted before the build; the builder is Opus 5 at both bands: high at band 2, xhigh at band 3); bands 0–1 are `· fableplan: no` (they don't need a separate plan). Always write it explicitly — absence is ambiguous, not "no".
