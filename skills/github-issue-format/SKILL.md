...
- First line of the body is a one-line rationale matching the title prefix, ending with an explicit **fableplan signal**:
  ...
  (Fable 5 never pairs with `xhigh`; the LLM Attribution Footer section of CLAUDE.md owns the Fable effort ceiling. xhigh is legal only on Opus/Sonnet-class builds.)
