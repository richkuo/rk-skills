...
- **fableplan signal:** `· fableplan: yes` when the score is ≥ 71 (a Fable 5.1 plan is posted before the build; the builder is Opus 5 at xhigh); `· fableplan: no` below 71. Always write it: absence is ambiguous. Fable 5.1 defaults to `high`; `xhigh` on Fable is legal only when the user asks for it or stamps it (the LLM Attribution Footer section of CLAUDE.md owns this rule).
