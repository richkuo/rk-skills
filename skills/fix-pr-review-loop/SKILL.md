...
| Rule 3 fired — sustained `Needs Updates` on self-inflicted findings | **Diverging — escalate the scope call.** Report the cycle count, the cumulative diff against the first push (`git diff --stat <first-push-sha>..HEAD`), and the chain: for each cycle, the finding it fixed and the finding that fix produced. Name the mechanisms the PR has grown that the issue it closes never asked for. Recommend one of: revert to the last cycle whose findings were all in the original work and file the rest as issues; or narrow the PR to the issue's acceptance criteria and file the remainder. Do not start another cycle. |
