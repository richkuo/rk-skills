...
Invoke the `fix-pr-review` skill (Skill tool, `skill: fix-pr-review`). It validates every finding, fixes what is real, resolves merge conflicts (its step 7), commits, pushes, posts the disposition comment, and posts its own re-review trigger (its step 10); never add a second one. It decides its own delegation (its step 5). Act on the outcome its step 10 reports. **Complete**: when it posted a trigger, increment `review_count`, record that trigger's timestamp, and go to step 2. **Pending trigger** (an earlier request it did not repeat): record that trigger's timestamp without incrementing and go to step 2. **Blocked**: go to step 5, **Blocked** row; start no further cycle. Any other pass that posted no trigger increments nothing and records no timestamp; the count reflects only the cycles step 2 waited on.
...
| fix-pr-review reported a blocked pass for any other reason | **Blocked.** Link the disposition, name each Blocked finding with the missing fact, access, or decision, and state what was pushed; no re-review was requested, and the maintainer decides before the next pass. |
