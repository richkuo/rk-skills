...
- **Title:** match the repo's PR-title convention (the commit-title style is usually right), plus a trailing `[C<score>, <model>, <effort>]` suffix — reuse the score from the issue's `[C<score>]` title prefix, paired with the model/effort actually used to build the PR (append `, fableplan` inside the bracket if a Fable plan ran first).
