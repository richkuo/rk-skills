...
- **Update existing tests the change makes stale.** When the change deliberately replaces behavior an existing test asserts, rewrite that test to assert the new correct behavior, and name the changed tests and the reason in the PR body (step 6). This covers fixtures, snapshots, and helper expectations, as well as assertions. A test that fails because the change is wrong stays as it is: fix the code. Never delete, skip, loosen, or narrow a test to get a green tree. The CLAUDE.md/AGENTS.md "tests are a correctness floor" rule governs, so a test you believe is itself wrong needs the maintainer's explicit sign-off before you touch it.
...
- **Changed existing tests:** when the diff rewrites, renames, or removes an existing test, list each one and say why the change made its old expectation wrong. A reviewer must be able to tell a deliberate behavior change from a weakened test.
...
| Existing test asserts behavior the change deliberately replaced | Rewrite the test to the new correct behavior and record it in the PR body; a test that fails because the code is wrong gets a code fix, and a test you believe is itself wrong needs maintainer sign-off before you touch it |
