...
- **Update existing tests the change makes stale.** The CLAUDE.md/AGENTS.md rule "The optimal solution comes first, and the tests follow it" owns this, needs no sign-off, and covers fixtures, snapshots, and helper expectations as well as assertions. Edit an existing test only under one of its three cases, and **name that case's checkable ground before you touch the test** — the issue, an acceptance criterion or spec, the code path or contract, a review finding, or a maintainer instruction. **Outdated:** the change deliberately replaces the behavior the test asserts, so rewrite it to the new behavior on the authority that authorized the change. **Wrong:** the test asserts behavior that was never correct, so rewrite it on the authority of whatever contradicts the old expectation. **Obsolete:** the change deletes the covered behavior outright, or another test now asserts it — remove the test and say which. For the redundancy case, read the surviving test first, name it, and confirm it carries every assertion of the removed one: each counterpart must reach the real code path rather than a stub or a mock, and must be able to fail on a defect the removed assertion caught. Keep the removed test when any assertion has no such counterpart. Disclose every edit in the commit message and the PR body (step 6).
- **A test with no ground stays as it is, and the code gets the fix.** A failing test is evidence, and the default reading is that the code is wrong. Your own judgment that a test asserts the wrong behavior is the claim under test and never its own ground, so it authorizes nothing on its own. **A test that breaks in another location is checked before it is edited:** read it and decide whether it is Outdated (the change deliberately replaces the tested behavior), Obsolete (the behavior is deleted, or another test covers it), or Wrong (the test was never correct, and passed only because the code shared its defect); one of those is rewritten or removed under that case with its ground, and a test that is none of the three means the change broke real behavior, so the code gets the fix. Not every change needs a new test; write one when it guards behavior that can regress. **The issue's scope decides nothing here:** a test outside the issue's scope with a real ground is rewritten like any other, and a test inside it with no ground is left alone. Never delete, skip, loosen, or narrow a test to get a green tree — that ban survives all three cases above. In the rare case where the correct change still cannot make an ungrounded test pass, step 4's gate holds: don't commit over the failure. Stop before step 5, keep the worktree, and report the test, its `file:line`, what it asserts, and why the correct change conflicts with it (step 7).
...
- **Changed existing tests:** disclose every test edit per the CLAUDE.md/AGENTS.md rule — name the test, which of the three cases it falls under (Outdated, Wrong, or Obsolete), that case's checkable ground, and what the replacement asserts. A removal gives the ground in place of the replacement, and for the redundancy ground it names the surviving test. The commit message carries the same disclosure. A reviewer must be able to tell a deliberate behavior change from a weakened test.
...
**When step 3 stopped the run** because no ground authorizes editing a test and the correct change cannot make it pass, this report replaces the normal one: say that no commit and no PR exist, name the test and its `file:line`, state what it asserts and why the correct change conflicts with it, and keep the worktree.
...
| Existing test asserts behavior the issue's change deliberately replaced | Rewrite it to the new correct behavior — or remove it only when the behavior is gone or another test now carries every assertion — name the checkable ground, and disclose it in the commit message and the PR body |
| A failing test looks wrong to you and no checkable ground says so | Leave it and fix the code — your reading is not a ground. If the correct change still cannot make it pass, don't commit: stop before step 5, keep the worktree, and report the test with its `file:line` and the conflict |
