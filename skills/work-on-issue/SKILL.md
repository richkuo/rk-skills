...
**Then scan the same comments for an open test sign-off request.** A prior run stops when a test outside the issue's scope looks wrong to it (step 3), recording that stop as an issue comment headed `## Test sign-off request`. The thread is the channel the answer arrives on, and the only one a later run can read — an autonomous re-invocation has no chat history. On the **newest** such request: a maintainer reply authorizing the change puts that test **inside this run's scope**, so rewrite it under step 3's first bullet and cite the approving comment's URL beside it in the PR body; a reply saying the test is right means the previous run's implementation was wrong, so build the change so that test passes; either way, never stop on that test again. No reply, or a reply that answers neither way, stops the run here before any worktree exists — report that the request is still open, never post a second request for the same test, and never proceed past it on your own authority.
...
| Issue thread carries a `## Test sign-off request` from a prior run | Step 0 decides: an authorizing reply puts that test in scope, a reply defending the test means build so it passes, and no reply stops the run again with no second request posted |
