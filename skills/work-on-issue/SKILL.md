...
One of: nothing (the issue validated this session, else `gh issue list --limit 1`); `#<N>` / `<N>` / URL / `owner/repo#N`; or the orchestration form `{ issue: <N>, baseRefs: [{ pr, ref, sha }, ...] }`, where `baseRefs` order is authoritative and each entry pins a predecessor PR's reviewed head. If the issue lives in another repo, work in a local clone of it, or stop and say which repo is needed.
