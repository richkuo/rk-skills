...
An **over-band build model or over-tertile build effort is not in this table at all**, by construction: both are observations under step 2(b)'s floor rule, printed as findings-table `Over-band` rows, and neither ever carries a severity or a downgrade recommendation.
