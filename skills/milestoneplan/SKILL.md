...
- **Plan** — `Yes · high` when fableplan is stamped `Yes`; plain `No` when stamped `No` (plan effort is never read), *missing* when the fableplan stamp is absent. A legacy stamped tier below `high` renders as `<tier> (ignored — fableplan always runs at high)`.
