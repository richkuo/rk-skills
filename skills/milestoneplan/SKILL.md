...
- **Review** — the effective first-review routing: the band default derived from the score (`Opus 5 · high` at 0–49, `Fable 5 · high` at 50+ or no prefix — cross-model against the builder where a stronger tier exists), unless the `PR review:` line stamps an explicit `@claude <model> review effort:<tier>` trigger, which overrides it; append a short parenthetical only when the line carries a real caveat (e.g. `may close with no PR`).
