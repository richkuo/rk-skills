...
- A referenced PR resolving `state: MERGED` → for an **in-milestone** predecessor the edge is **satisfied** (drop it; no finding). For an **out-of-milestone** predecessor the reference is still rejected by step 1 — severity follows the closed cross-milestone rows, never this satisfied carve-out.
...
**(d) Readiness.** Cross-milestone prerequisites — recorded separately by edge kind **and** by whether the predecessor is still open, because `milestone-workflow` step 1 rejects *every* out-of-milestone reference (the resolve carve-out is in-milestone only). An open **hard** cross-milestone prerequisite means the code does not exist yet *and* the runner rejects the reference; a **closed** hard one (merged or not) is still a rejected reference — exclude until the stamp is rewritten. An open **ordering-only** one is merely unenforceable in `tracks`; a closed ordering-only one likewise needs the reference dropped. Also flag: issues whose hard predecessors all sit in a later milestone; an issue already closed but still carrying open dependents.
...
| A **closed** **hard** cross-milestone predecessor (merged or not) | **Blocked — excluded** | step 1 rejects every out-of-milestone reference — the in-milestone "merged → satisfied" carve-out does not apply. Unblocks when the stamp drops the out-of-milestone number (typically rewrite to `none` when the predecessor already merged) |
...
| A **closed** **ordering-only** cross-milestone predecessor (merged or not) | **Non-blocking** | same rejection rule; recommend dropping the out-of-milestone number (stamp `none` when already merged) |
...
| **Any row above whose owning issue sits in the skip or resume bucket** — step-1 blocking unknowns excepted (incomplete fetch, failed/throttled/truncated open-PR query, undecidable linked-reference openness, unfetched out-of-milestone reference): those findings say the bucket itself is undecidable, so demotion is unevaluable and must stay NO-GO. Never a cross-bucket edge row: an edge finding is owned by its runnable dependent (step 2's ownership rule), which *is* dispatched | **Informational** | a finding owned by a skip- or resume-bucket issue cannot decide the verdict — neither bucket is dispatched this run (step 2's scoping rule). For a resume issue, name the re-entry: its finding becomes blocking if that PR closes unmerged |
...
| Ordering fields missing, prose gestures at a dependency but does not establish kind | Non-blocking — flag for plan review the same way `milestone-workflow` step 1 does; never NO-GO. Route to `execution-plan-review` |
| A hard `Depends on` names a closed issue outside this milestone (merged or not) | Blocked — excluded — step 1 rejects every out-of-milestone reference; rewrite the stamp (usually to `none` when already merged) |
