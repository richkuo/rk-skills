...
**Establish "open" by intersecting with the open-PR list — the field carries no PR state.** As this step says below, `closedByPullRequestsReferences` names the PRs that close each issue but **not** whether they are open or merged. So: an issue is **resume** when any of its reference numbers appears in the open-PR list fetched above (same-repo references only — match on `number`). A same-repo reference whose number is *not* in that list is not open; do not treat it as resume from the field alone — fall through to the keyword scan. A reference whose openness cannot be established from that list — notably a **cross-repo** reference, whose `repository` is not this repo — is a **blocking unknown**, never assumed open and never assumed closed: assuming open would send a dead PR into `fix-pr-review-loop`, and assuming closed would collapse back to the keyword scan the sidebar-linkage rule exists to replace. Then scan the fetched open-PR bodies with the pattern below for anything the field did not report.
...
| A closing PR reference's openness cannot be established (cross-repo in `closedByPullRequestsReferences`) | Blocking unknown — never assume open (would resume a dead PR) and never assume closed (would miss a sidebar-linked open PR) |
