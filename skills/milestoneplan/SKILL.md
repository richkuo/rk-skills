...
- **Review** — the effective first-review routing: the band default derived from the score (`Opus 5 · high` at 0–74, `Fable 5 · high` at 75+ or no prefix), unless the `PR review:` line stamps an explicit `@claude <model> review effort:<tier>` trigger, which overrides it; append a short parenthetical only when the line carries a real caveat (e.g. `may close with no PR`).
