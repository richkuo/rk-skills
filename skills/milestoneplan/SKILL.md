...
- **Resume bucket** → **Informational** for any finding that would be **NO-GO** on a runnable issue (deferred — becomes blocking if that PR closes unmerged). Stamp/band contradictions and other Non-blocking findings keep **Non-blocking** so step 6's recommendations still land — give them findings-table severity `Non-blocking` with the re-entry named (`→ becomes blocking if PR #<n> closes unmerged`). Its PR runs through `fix-pr-review-loop` *outside* the pipeline, so its Execution block is not read this run either; the stamp fix is still worth applying now.
...
- **Recommended fix** — the corrected value, not a description of it, followed in parentheses by the band/tertile derivation that produced it, so the reader can check the recommendation without recomputing the score (`Effort: xhigh (Volume 21 → 16–24 tertile)`, `Model: Fable 5 (C81 → Capability 3)`). A Validate-effort fix cites Validate effort's own medium-or-high rule, never the Volume tertile; a fix that does not derive from the score (an edge correction like `Depends on: #12`) cites the evidence instead (`(hard dependency stated in body)`). Where there is no value to correct, the cell takes one of two fixed forms instead, never a free-form description and never blank. **`add — <what is missing>`** when the fix is supplying something absent (`add — acceptance criteria`, `add — complexity rationale line`, `add — [C<score>] title prefix`). **`none — <what disposes of the finding>`** when no write clears it: a `Blocked — excluded` row carries its unblock condition (`none — unblocks when PR #X merges`, `none — blocked pending decision`); an `Informational` row carries its disposition — `none — never re-enters` on a skip-bucket issue (step 6 forbids recommending a change to a body this run will never read), `none — satisfied by sequencing` for an ordering-only edge into the resume bucket, `none — not dispatched this run` plus the re-entry condition for a finding demoted from a NO-GO class.
- **Route** — the skill that owns the fix, per step 6: `execution-plan-review`, `validate-issue`, or `—` when no write clears it. Those are step 6's only two fix owners; never route a finding anywhere else.
...
| **Any NO-GO-class finding whose owning issue sits in the resume bucket or a *Blocked — excluded* subtree** — Non-blocking stamp/band findings on those issues stay Non-blocking (step 6 still recommends; for resume, name the re-entry) | **Informational** | the issue is not dispatched this run (step 2's resume / excluded scoping rules). Name the re-entry: resume → blocking if that PR closes unmerged; excluded → blocking once the blocker clears |
...
**The recommendations live in the step-5 findings table's `Recommended fix` and `Route` columns — one row per contradicting field, never a second prose list restating them here.** One field, one row: an issue contradicting three stamps gets three rows, not one row naming three fixes, so each fix is independently checkable and applicable. Recommend a fix on a **build-bucket issue that is runnable, *Blocked — excluded*, or resume-bucket**. Excluded issues re-enter the runnable set once their blocker clears, and resume issues return to the build bucket if their PR closes unmerged — so fix their stamps now rather than leaving an `Informational` row blank for a finding the report itself says will become blocking. Field contradictions on **skip**-bucket issues take severity `Informational` and `Recommended fix: none — never re-enters` — those skills would edit a body this run never reads and that will not re-enter.
...
| A **resume-bucket** stamp/band contradiction | Non-blocking — keep the recommendation (step 6) and name the re-entry; same carve-out *Blocked — excluded* already has |
