...
**Fetch every referenced issue that is not already in the milestone set — the severity table turns on its existence, state, and milestone.** Collect the distinct issue numbers named in every `Depends on` / `Runs after` (and any prose-inferred edges later labeled as such), drop those already fetched above, and look up the rest in one bounded batch — one request per ~50 numbers, never a repo-wide list:

```
gh issue view <n> --json number,state,stateReason,milestone
```

Or the batched GraphQL equivalent (`issue(number:N){ number state stateReason milestone{ title number } }`), aliased like the closing-PR lookups. Same-repo only here; a cross-repo issue reference still uses `-R <owner>/<repo>` per the failure-mode row below. A returned issue outside this milestone is a **cross-milestone predecessor** — use its `state` and `milestone` for the readiness rows, never treat absence-from-the-milestone-fetch as "does not exist". A number that returns nothing is a genuinely nonexistent reference. A lookup that errors, throttles, or cannot be completed is a **blocking unknown** — never score it as nonexistent (that would NO-GO a healthy cross-milestone edge). The request count is bounded by the distinct out-of-milestone references on this milestone's issues, not by repository size.
