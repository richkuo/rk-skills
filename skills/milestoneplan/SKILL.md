...
- **Plan** — when fableplan is stamped `Yes`, show `Yes · high` as the effective value; when a legacy `Plan effort:` stamp exists and is not `high`, append ` (<stamped tier> ignored — fableplan always runs at high)` so the row reads e.g. `Yes · high (xhigh ignored — fableplan always runs at high)`. Plain `No` when stamped `No`; *missing* when the fableplan stamp is absent. On `No` issues, a legacy stamped tier that is not `high` renders as `<tier> (ignored — fableplan always runs at high)` in the Plan effort column.
