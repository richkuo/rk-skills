...
- `state: OPEN` → its code is not in the base branch yet; treat it as the resume case, not as merged.
...
```
gh api graphql -F owner=':owner' -F repo=':repo' -f query='query($owner:String!,$repo:String!){ repository(owner:$owner,name:$repo){
  i42: issue(number:42){ timelineItems(first:100, itemTypes:[CROSS_REFERENCED_EVENT]){ pageInfo{hasNextPage endCursor}
...
- #N — <stamped model> against a Capability-<n> band → <what it costs, why it may be intended>
...
| A stamp contradicting its band (an under-band build model, a build effort off its Volume tertile, `fableplan` set against its Capability band, or a Validate effort off its own medium-or-high rules), an inert field (a `Plan effort` stamped on a `fableplan: No` issue), a stale build model name that no longer maps, or a stamp that lies about what will run (a non-Fable build stamped `low`/`medium`, or `Validate effort: xhigh` — the runtime silently normalizes these to `high`) | **Non-blocking** | the run proceeds; only the paperwork is wrong. A stale model name belongs here because prep's output schema forces one of the four model ids, so an unmappable name gets coerced to whichever of the four the prep agent picks — unpredictably, since the `fable` default applies only to a missing Execution block, which has its own NO-GO row — and the stamp misreports what will actually run |
...
An **over-band build model is not in this table at all**, by construction: it is an observation under step 2(b)'s floor rule, printed under *Over-band observations*, and it never carries a severity or a downgrade recommendation.
...
**Only the runnable build-bucket rows are what the run will execute.** Mark every other row with its bucket and say plainly that it is not dispatched — never present a closed or resume row as pending pipeline work. Mark inferred values as inferred and missing ones as *missing* (never blank, never a guessed default), and note that the pipeline would route a missing Execution block to `model fable, effort high`.
...
Write each recommendation as one line per contradicting field, on a **runnable build-bucket issue only**: `#N — <field>: <stamped> → <recommended> (<why, derived from the score>)`. Field contradictions on skip- or resume-bucket issues print under *Informational*, never as change recommendations routed to `execution-plan-review` or `validate-issue` — those skills would edit a body this run never reads.
...
| An issue is stamped a model above its band | Observation, never a downgrade recommendation — the band is a floor. Print it under *Over-band observations*; annotated, it is a deliberate override |
| A Fable build is stamped `low` | On-rule — the discretionary Fable-only tier below the formula floor. Never flag it |
